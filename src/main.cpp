...
static const std::uint8_t deveui[] = DEVEUI;

// appeui, little-endian
static const std::uint8_t appeui[] = APPEUI;

// appkey: just a string of bytes, sometimes referred to as "big endian".
static const std::uint8_t appkey[] = APPKEY;
...
cSensor::setup(std::uint32_t uplinkPeriodMs) {
    Wire.begin(4,15);
    if (! this->m_BME280.begin(0x76, &Wire)) {
        while (true) {
    ...
    this->m_BME280.setSampling(this->m_BME280.MODE_FORCED);
...
cSensor::doUplink(void) {
    ...
    Serial.print("Forced="); Serial.print(this->m_BME280.takeForcedMeasurement());
    auto const m_Temperature = this->m_BME280.readTemperature();
    auto const m_Pressure = this->m_BME280.readPressure();
    auto const m_Humidity = this->m_BME280.readHumidity();

    Serial.print("Sensor: T="); Serial.print(m_Temperature);
    Serial.print(" degC, P="); Serial.print(m_Pressure);
    Serial.print(" hPa, RH="); Serial.print(m_Humidity);
    Serial.println("%");
    ...
    auto const t = m_Temperature;
    auto const it = std::int16_t(floor(t * 256 + 0.5));
    auto const up = std::uint32_t(floor(m_Pressure + 0.5));
    auto const uh = std::uint16_t(m_Humidity / 100.0 * 65535 + 0.5);
